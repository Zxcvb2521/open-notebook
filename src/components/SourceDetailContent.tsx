...
/** Safe date formatting — never throws on invalid/missing timestamps. */
function safeFormatDistance(value: string | null | undefined, language: string): string {
  if (!value) return '—'
...
function safeLocaleString(value: string | null | undefined): string {
  if (!value) return '—'
  const date = new Date(value)
  if (Number.isNaN(date.getTime())) return '—'
  try {
                ...
                <div className="border-t border-border pt-5">
                  ...
                  <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                      ...
                      <p className="text-sm">
                        {safeFormatDistance(source.created, language)}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {safeLocaleString(source.created)}
                      </p>
                    </div>
                    <div>
                      <p className="text-xs font-medium text-muted-foreground">{t('common.updated_label')}</p>
                      <p className="text-sm">
                        {safeFormatDistance(source.updated, language)}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {safeLocaleString(source.updated)}
                      </p>
                    </div>
                  </div>
                </div>
